...
// UNUSED EXPORTS: doesTableExist, generateSqlQueryFromNaturalLanguage
...
// EXTERNAL MODULE: ./src/sql.ts + 1 modules
var sql = __webpack_require__(144);
...
;// CONCATENATED MODULE: ./src/chatbot.ts
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const tableSelect = document.querySelector('#table-selection');
    let UIState;
    (function (UIState) {
        UIState[UIState["WaitingForInput"] = 0] = "WaitingForInput";
        UIState[UIState["WaitingForGptResponse"] = 1] = "WaitingForGptResponse";
    })(UIState || (UIState = {}));
    let uiState = UIState.WaitingForInput;
    let messages = [];
    ...
    async function handleMessageSend() {
        if (uiState !== UIState.WaitingForInput)
            return;
        ...
        try {
            ...
            while (true) {
                ...
                const response = await fetch('https://api.openai.com/v1/chat/completions', {
                    ...
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": `Bearer ${await (0,ApiKeys/* getOpenAiApiKey */.u4)()}`
                    },
                    body: JSON.stringify({
                        ...
                        switch (tool_call.function.name) {
                            case "run_sql":
                                if (argument_object.statement !== undefined) {
                                    const result = await (0,sql/* runSql */.Jh)(argument_object.statement);
                                    messages.push({
                                        role: 'tool',
    ...
    function addToChatsContainer(message) {
        const div = document.createElement('div');
        div.innerHTML = marked.parse(message, { async: false });
    ...
    async function resetConversationForTable(tableName) {
